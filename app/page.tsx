...
import { ArrowLeft, ArrowRight, ArrowUpRight, Bell, Home, LineChart, Plus, Settings } from "lucide-react"
import Image from "next/image"

export default function CryptoApp() {
  const [currentScreen, setCurrentScreen] = useState<"welcome" | "portfolio" | "trade">("welcome")

  const renderScreen = () => {
    switch (currentScreen) {
      case "welcome":
        return <WelcomeScreen onNext={() => setCurrentScreen("portfolio")} />
      case "portfolio":
        return <PortfolioScreen onTrade={() => setCurrentScreen("trade")} />
      case "trade":
        return <TradeScreen onBack={() => setCurrentScreen("portfolio")} />
      default:
        return <WelcomeScreen onNext={() => setCurrentScreen("portfolio")} />
    }
  }
  ...
  return (
    <main className="min-h-[100dvh] bg-gray-50">
      <div className="max-w-md mx-auto bg-white min-h-[100dvh]">{renderScreen()}</div>
    </main>
  )
}
...
function WelcomeScreen({ onNext }: { onNext: () => void }) {
  return (
    <div className="relative h-[100dvh] overflow-hidden bg-gradient-to-br from-blue-600 to-blue-700 text-white p-8 flex flex-col justify-between">
      ...
      <Button variant="secondary" size="lg" className="w-12 h-12 rounded-full p-0 mx-auto mb-8" onClick={onNext}>
...
function PortfolioScreen({ onTrade }: { onTrade: () => void }) {
  return (
    <div className="min-h-[100dvh] flex flex-col">
      <div className="h-8 bg-white" /> {/* Status bar space */}
      <div className="flex-1 p-6">
        ...
        <div className="mb-6">
          ...
          <div className="space-y-4">
            <CryptoItem name="Bitcoin" symbol="BTC" amount="1,132,151" change="+2.35%" onClick={onTrade} />
            <CryptoItem name="Ethereum" symbol="ETH" amount="1,132,151" change="+2.35%" onClick={onTrade} />
          </div>
        </div>

        <Button size="lg" className="w-14 h-14 rounded-full fixed bottom-24 right-6 p-0 shadow-lg" onClick={onTrade}>
...
function CryptoItem({
  ...
  return (
    <button
      className="flex items-center justify-between p-4 bg-gray-50 rounded-xl w-full hover:bg-gray-100 transition-colors"
      onClick={onClick}
    >
